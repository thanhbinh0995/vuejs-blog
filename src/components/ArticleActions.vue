...
import { ARTICLE_TYPE, USER_TYPE } from "@/types";
  ...
  methods: {
    toggleFavorite() {
      ...
      const action = this.article.favorited
        ? ARTICLE_TYPE.FAVORITE_REMOVE
        : ARTICLE_TYPE.FAVORITE_ADD;
    ...
    toggleFollow() {
      ...
      const action = this.article.following
        ...
        : USER_TYPE.FETCH_PROFILE_FOLLOW;
