...
export default {
  ...
  async beforeRouteUpdate(to, from, next) {
    ...
    await store.dispatch(ARTICLE_TYPE.ARTICLE_RESET_STATE);
    return next();
  },
  ...
  methods: {
    onPublish(slug) {
      let action = slug
        ? ARTICLE_TYPE.ARTICLE_EDIT
        : ARTICLE_TYPE.ARTICLE_PUBLISH;
    ...
    removeTag(tag) {
      this.$store.dispatch(ARTICLE_TYPE.ARTICLE_EDIT_REMOVE_TAG, tag);
    },
    addTag(tag) {
      this.$store.dispatch(ARTICLE_TYPE.ARTICLE_EDIT_ADD_TAG, tag);
      this.tagInput = null;
    }
  }
};
